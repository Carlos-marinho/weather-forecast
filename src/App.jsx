...
function App() {
  ...
  async function handleSearch() {
    try {
      ...
      const api = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${API_KEY}&units=metric&lang=pt_br`)
      const api5days = await axios.get(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&appid=${API_KEY}&units=metric&lang=pt_br`)
      setWeather(api.data)
  ...
  return (
    <div className='w-screen h-screen bg-blue-400'>
      <div className="flex flex-col w-8/12 mx-auto justify-center items-center gap-y-6">
